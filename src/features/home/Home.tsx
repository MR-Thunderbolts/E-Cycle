...
const Home: React.FC<HomeProps> = ({ onOpenProfile }) => {
    ...
    return (
        <div className="relative w-full h-full bg-[#F2F1ED] dark:bg-dark-bg overflow-hidden font-sans">
                ...
                {isSearchOpen && (
                    ...
                    >
                        <div className="px-6 pt-6 pb-2 flex justify-between items-center bg-transparent shrink-0">
                            <button onClick={() => setIsSearchOpen(false)} className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-dark-surface shadow-sm text-text dark:text-dark-text">
                                        ...
                                        <div className="grid grid-cols-4 gap-3">
                                            {RECYCLE_CATEGORIES.map(cat => (
                                                <button key={cat.id} onClick={(e) => { e.stopPropagation(); toggleMaterial(cat.id); }} className={`flex flex-col items-center gap-3 p-3 rounded-3xl transition-all ${selectedMaterials.includes(cat.id) ? 'bg-[#E0F2F1] dark:bg-primary/20 ring-1 ring-primary' : 'bg-white dark:bg-dark-surface shadow-sm'}`}>
                                                    <span className={`material-symbols-rounded text-2xl ${selectedMaterials.includes(cat.id) ? 'filled-icon text-primary' : 'text-gray-400'}`}>{cat.iconName}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </motion.div>
                        ...
                        <div className="p-4 bg-white dark:bg-dark-surface border-t border-gray-100 dark:border-dark-border flex justify-between items-center shrink-0 pb-safe-bottom">
                            <button onClick={() => { setSearchQuery(''); setSelectedMaterials([]); setActiveSearchTab('donde'); }} className="font-bold text-sm underline px-4">Limpiar todo</button>
            ...
            >
                <div className="bg-white dark:bg-dark-surface rounded-[32px] shadow-lg p-2 flex items-center gap-3 border border-transparent dark:border-dark-border">
                    <button onClick={() => setIsSearchOpen(true)} className="flex-1 flex items-center h-12 pl-4 text-left">
                        <span className="material-symbols-rounded text-primary text-3xl mr-3 font-bold">search</span>
            ...
            <motion.div
                ...
                className="absolute bottom-44 right-4 flex flex-col gap-3 z-10"
            >
                <div className="bg-white dark:bg-dark-surface rounded-[32px] shadow-lg flex flex-col">
                    <button onClick={() => setMapZoom(z => Math.min(z + 0.5, 4))} className="w-12 h-12 flex items-center justify-center text-primary border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-white/5 rounded-t-[32px] transition-colors"><span className="material-symbols-rounded text-2xl">add</span></button>
                    <button onClick={() => setMapZoom(z => Math.max(z - 0.5, 0.5))} className="w-12 h-12 flex items-center justify-center text-primary hover:bg-gray-50 dark:hover:bg-white/5 rounded-b-[32px] transition-colors"><span className="material-symbols-rounded text-2xl">remove</span></button>
                </div>
                <button onClick={() => { setCenterMapOnUser(true); setMapZoom(1.5); }} className="w-12 h-12 bg-white dark:bg-dark-surface rounded-full shadow-lg flex items-center justify-center text-primary hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"><span className="material-symbols-rounded filled-icon text-2xl">my_location</span></button>
            </motion.div>
                    ...
                    <>
                        ...
                        >
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-4 bg-white dark:bg-dark-surface border-t border-gray-50 dark:border-dark-border flex gap-3 z-30 pb-8 rounded-t-[20px]">
                                <button className="flex-1 bg-[#00897B] text-white py-4 rounded-full font-bold text-sm shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all">
                                    <span className="material-symbols-rounded text-xl">qr_code_scanner</span> Escanear QR
                                </button>
                                <button className="flex-1 bg-[#004D40] text-white py-4 rounded-full font-bold text-sm shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all">
